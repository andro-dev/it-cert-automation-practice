...
def get_file_data(url):
    ...
    dates = {row[3] for row in rows}  ## unique dates
    getNames = lambda sdate: [f'{r[0]} {r[1]}' for r in rows if r[3] == sdate]
    return {f"{row[0]} {row[1]}": datetime.datetime.strptime(f"{row[3]}", '%Y-%m-%d') for row in rows}
    # {datetime.datetime.strptime(d, '%Y-%m-%d'): getNames(d) for d in dates}
...
def list_newer(start_date, end_date=datetime.datetime.today(), data=employee_dates):
      newer_list = {k: v for k, v in sorted(data.items(), key=lambda item: item[1])}
    for k in newer_list:
        print(k, newer_list.get(k).strftime('%b %d, %Y'))
    # newer_list = sorted([item for item in employee_dates if start_date <= employee_dates.get(item) < end_date], key=lambda item:item[1])  # filter+sort
    # for key in newer_list:
    #     print(f"{key}: started on {data[key].strftime('%b %d, %Y')}")
